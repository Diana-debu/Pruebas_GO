...
class FinalLmpData(BasicCal):
    def __init__(self, filename, add_z, add_y=[]):
        ...
        self.box = []
        self.addzlen = 10.0
# ODOG - Al parecer desde aquí ya comienza a crear la estructura (LO QUE ME INTERESA) (OJO, a partir de la estructura de la pirofilita, que sirve para la Wyoming)
        for layer in range(len(add_z)): # ODOG - Ciclo para añadir moléculas y átomos en la dirección z (ver Ovito) (Esta es la dirección de interés). Cada iteración hace referencia a un espacio intercapa (en donde se añadirán las moleculas/átomos de na y h2o)
            ...
            for ix in range(len(add_z[layer])): # ODOG - En este ciclo se busca en el directorio actual las moléculas que se quieren añadir (na y h2o)
            ...
            for ix in range(len(add_z[layer])): # ODOG - También relacionado con las moléculas que se quieren agregar (na y h2o)
            ...
            print("now packmol:", end=' ') # ODOG - En este punto ya se tiene el input para packmol (z.inp)
            # ODOG - Comando a ejecutar en Ubuntu - Línea insertada por ODOG
            command = "/mnt/c/users/public/nuevacarpeta/tesis/Estructuras/packmol-20.14.3/packmol < z.inp > /dev/null" # ODOG - Aquí se especifica el comando que se insertará dentro de la consola Ubuntu. Línea insertada por ODOG. El /dev/null es para que el packmol no muestre la salida en pantalla.
            # ODOG - Se llama a la terminal de Ubuntu (Linux) usando wsl.exe y se ejecuta el comando anterior (command)
            subprocess.run(["C:\\Windows\\System32\\wsl.exe", "bash", "-c", command]) # ODOG - Aquí se llama al Windows Subsystem for Linux (WSL - Linux - Ubuntu) y se ejecuta el comando (command). Línea insertada por ODOG. Se trabaja sobre el directorio de trabajo actual en Python (Confirmado). '"bash", "-c"' le dice a WSL que ejecute el comando proporcionado usando bash. Bash proporciona una interfaz de línea de comandos para interactuar con el sistema operativo, permitiendo a los usuarios ejecutar comandos, automatizar tareas mediante scripts y trabajar con archivos y directorios en el sistema de archivos.
            #os.system('~/apps/packmol/packmol < z.inp > /dev/null') # ODOG - Aquí se llama a packmol a travez del cmd y se ejecuta a partir del z.inp. Línea comentada, su función es hecha en las dos líneas anteriores (Insertadas por ODOG).
            print("packmol has been done;", end=' ')
            self.readxyz("z.xyz") # ODOG - Guarda/agrega el z.xyz generado por el packmol en tal iteración
            # ODOG - Se borran tanto el input (z.inp) como la salida del packmol (z.xyz) para tal iteración. Línea insertada por ODOG.
            subprocess.run(["C:\\Windows\\System32\\wsl.exe", "bash", "-c", "rm z.inp z.xyz"]) # ODOG - Borrar lo obtenido en tal iteración (No se pierde, pues ya se guardó). Lo borra del directorio actual de trabajo en Python (Confirmado). '"bash", "-c"' le dice a WSL que ejecute el comando proporcionado usando bash. Bash proporciona una interfaz de línea de comandos para interactuar con el sistema operativo, permitiendo a los usuarios ejecutar comandos, automatizar tareas mediante scripts y trabajar con archivos y directorios en el sistema de archivos.
            #os.system('rm z.inp z.xyz') # ODOG - Se borran tanto el input (z.inp) como la salida del packmol (z.xyz) para tal iteración (recordar que cada iteración a este nivel, hace referencia a cada espacio intercapa). Línea comentada, su función es hecha en la línea anterior (Insertada por ODOG)
            print("all atoms has been read;")
        for layer in range(len(add_y)): # ODOG - Hace algo parecido al ciclo de igual nivel anterior (add_z), pero esta vez para la dirección y (add_y). Pero para la Wyoming no es necesario agregar moléculas/átomos en tal dirección
            print("deal with the %d layer in y direction:" % layer)
            self.writeinp(direction="y", addmol=add_y[layer])
            for ix in range(len(add_y[layer])): # ODOG - En este ciclo se busca en el directorio actual las moléculas que se quieren añadir (na y h2o)
            ...
            for ix in range(len(add_y[layer])): # ODOG - También relacionado con las moléculas que se quieren agregar (na y h2o)
                self.molname.append(add_y[layer][ix][0])
                file2 = open(add_y[layer][ix][0] + '.xyz')
                self.molnum.append(len(file2.readlines()) - 2)
                file2.close()
                self.moltimes.append(add_y[layer][ix][3])
            print("now packmol:", end=' ') # ODOG - En este punto ya se tiene el input para packmol (z.inp). NO SE USA Y
            # ODOG - Comando a ejecutar en Ubuntu - Línea insertada por ODOG
            command = "/mnt/c/users/public/nuevacarpeta/tesis/Estructuras/packmol-20.14.3/packmol < y.inp > /dev/null" # ODOG - Aquí se especifica el comando que se insertará dentro de la consola Ubuntu. Línea insertada por ODOG. El /dev/null es para que el packmol no muestre la salida en pantalla. NO SE USA Y
            # ODOG - Se llama a la terminal de Ubuntu (Linux) usando wsl.exe y se ejecuta el comando anterior (command)
            subprocess.run(["C:\\Windows\\System32\\wsl.exe", "bash", "-c", command]) # ODOG - Aquí se llama al Windows Subsystem for Linux (WSL - Linux - Ubuntu) y se ejecuta el comando (command). Línea insertada por ODOG. Se trabaja sobre el directorio de trabajo actual en Python (Confirmado). '"bash", "-c"' le dice a WSL que ejecute el comando proporcionado usando bash. Bash proporciona una interfaz de línea de comandos para interactuar con el sistema operativo, permitiendo a los usuarios ejecutar comandos, automatizar tareas mediante scripts y trabajar con archivos y directorios en el sistema de archivos. NO SE USA Y
            #os.system('~/apps/packmol/packmol < y.inp > /dev/null')
            print("packmol has been done;", end=' ')
            self.readxyz("y.xyz") # ODOG - Guarda/agrega el y.xyz generado por el packmol en tal iteración. NO SE USA Y
            # ODOG - Se borran tanto el input (y.inp) como la salida del packmol (y.xyz) para tal iteración. Línea insertada por ODOG.
            subprocess.run(["C:\\Windows\\System32\\wsl.exe", "bash", "-c", "rm y.inp y.xyz"]) # ODOG - Borrar lo obtenido en tal iteración (No se pierde, pues ya se guardó). Lo borra del directorio actual de trabajo en Python (Confirmado). '"bash", "-c"' le dice a WSL que ejecute el comando proporcionado usando bash. Bash proporciona una interfaz de línea de comandos para interactuar con el sistema operativo, permitiendo a los usuarios ejecutar comandos, automatizar tareas mediante scripts y trabajar con archivos y directorios en el sistema de archivos. NO SE USA Y
            #os.system('rm y.inp y.xyz') # ODOG - Se borran tanto el input (y.inp) como la salida del packmol (y.xyz) para tal iteración (recordar que cada iteración a este nivel, hace referencia a cada espacio intercapa). Línea comentada, su función es hecha en la línea anterior (Insertada por ODOG). NO SE USA Y
            print("all atoms has been read;")
        self.calid()
        self.addbond_ang()
        self.caltype()
    ...
    def get_charge(ele_name):
        char_dic = {"st": 2.1000, "ao": 1.5750, "ob": -1.0500, "obts": -1.1688, "obos": -1.1808, "ohy": -0.9500,
                    ...
                    "li": 1.0000, "lix": 1.0000, "kx": 1.0000, "w_ow": -0.8476, "w_hw": 0.4238, "C": 0.0000}  # charge dictionary # ODOG - Edited, agregué el carbono del grafeno "C"
    ...
    def set_coeff(self):
        ...
        self.angcoeff = {"hw-ow-hw": ['harmonic', '33.43000', '109.47000'], "w_hw-w_ow-w_hw": ['harmonic', '33.43000', '109.47000'],
                         ...
                         "hoy-ohs-mgo": ["harmonic", '6.00000', '110.10000'], "HC-CT-HC": ["harmonic", '35.0000', '109.500'], "C-C-C": ["harmonic", '3.9028', '120.00']} # ODOG - Edited, agregué el carbono del grafeno "C"
        self.dihcoeff = {}
        self.mass = {"st": 28.08600, "at": 26.98150, "ao": 26.98150, "mgo": 24.30500, "emgo": 24.30500, "ob": 15.99940,
                     ...
                     "kx": 39.0983, "li": 6.9410, "lix": 6.9410, "C": 12.00000}  # mass dictionary # ODOG - Edited, agregué el carbono del grafeno "C" 
        self.paircoeff = {"st": [0.0000018405, 3.30203000], "at": [0.0000018405, 3.30203000],
                          ...
                          "lix": [0.1553000000, 1.4740000], "C": [0.0064179400, 4.0600000]}  # pair_coeff dictionary # ODOG - Edited, agregué el carbono del grafeno "C"
...
# FinalLmpData("./"+"pyro_xyz.txt", add_z=[[['na', 23, 5.0, 24], ["h2o", 18, 1.1, 30]],
#                                      [['na', 23, 5.0, 24], ["h2o", 18, 1.1, 30]],
#                                      [['na', 23, 5.0, 24], ["h2o", 18, 1.1, 30]],
#                                      [['na', 23, 5.0, 24], ["h2o", 18, 1.1, 30]]], add_y=[]) # ODOG - En el add_z, el cual es una lista anidada, por ejemplo, dentro del primer conjunto de elementos de la primera anidación, el "na" indica el nombre del archivo que contiene los átomos/moléculas que se van a agregar, el "23" es como un ~identificador de tal archivo, el "5.0" es la distancia que por lo menos debe tener un átomo/molécula agregada de otra agregada, y por último, el "24" indica la cantidad de átomos/moléculas que se agregarán.

FinalLmpData("./"+"pyro_xyz.txt", add_z=[[['na', 23, 5.0, 24]],
                                     [['na', 23, 5.0, 24]],
                                     [['na', 23, 5.0, 24]],
                                     [['na', 23, 5.0, 24]]], add_y=[]) # ODOG - En el add_z, el cual es una lista anidada, por ejemplo, dentro del primer conjunto de elementos de la primera anidación, el "na" indica el nombre del archivo que contiene los átomos/moléculas que se van a agregar, el "23" es como un ~identificador de tal archivo, el "5.0" es la distancia que por lo menos debe tener un átomo/molécula agregada de otra agregada, y por último, el "24" indica la cantidad de átomos/moléculas que se agregarán.
